...
;(def games (playGame player1 player2 (range 9)))
;(println games)
(def winners (map #(getWinner (first %)) games))
(println (frequencies winners))
;(println (/ (- (System/currentTimeMillis) ms) 1000.0))

;(def perm (permutations (range 9)))
;(def games (reduce #(assoc %1 (apply str %2) (playGame player1 player2 %2)) {} perm))
;(def allWins (filter isWon (vals games)))
;(println (frequencies (map count allWins)))
;(def allCenterWins
;  (map (fn [pos]
;    (count (filter
;      #(and (=  pos (first (key %)))
;        (not= (getWinner (val %))xPlayer))
;      games))) [\0 \1 \2 \3 \4 \5 \6 \7 \8]))
;(println allCenterWins)
;(def allCenterMoves
;  (filter
;    #(= \4 (first (key %)))
;    games))
;(print "center wins ")
;(println (count allCenterWins ))
;(print "center moves ")
;(println (count allCenterMoves))
;(def winners (map getWinner allWins))
;(def xWinners (filter #(= xPlayer %) winners))
;(print "x winners ")
;(println (count xWinners))
;(print "total wins ")
;(println (count allWins))
;(print "total games ")
;(println (count perm))